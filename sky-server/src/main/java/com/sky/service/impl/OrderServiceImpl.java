...
//        }
        ...
        vo.setPackageStr(jsonObject.getString("packageStr"));  //TODO:这里返回前端的vo消息有问题
...
//        webSocketServer.sendToAllClient(json);
    ...
    public void userCancelById(Long id) throws Exception {
        ...
        if(ordersDB.getStatus() > 2){ //已接单 4派送中是不能取消订单，需要电话商家
...
//                    new BigDecimal(0.01));//原订单金额
        ...
        List<ShoppingCart> shoppingCartList = orderDetailList.stream().map(x->
                {
                    ...
                    BeanUtils.copyProperties(x,shoppingCart,"id");//原订单详情的菜品信息复制到新的购物车对象中，但排除id属性
                    shoppingCart.setUserId(userId);
    ...
    public void rejection(OrdersRejectionDTO ordersRejectionDTO) throws Exception {
        ...
        //支付状态
        //payment statue
        Integer payStatue = ordersDB.getPayStatus();
        if(payStatue == Orders.PAID){
            //用户已支付，需要退款costumer has already made the payment,refund required
...
//            log.info("申请退款：{}", refund);
        ...
        //update rejection reason,ORDER statue,rejection time according to Order id
        //just update the fields you need,if using ordersDB,maybe have to update too much unrelated fields
        Orders orders =new Orders();
...
//            log.info("申请退款：{}", refund);
    ...
    public void delivery(Long id) {
        ...
        //only orderDB with a status of CONFIRMED(3) can be delivered
        Integer orderDBStatus = orderDB.getStatus();
        if(orderDBStatus==null || !orderDBStatus.equals(Orders.CONFIRMED)){
            throw new OrderBusinessException(MessageConstant.ORDER_STATUS_ERROR);
        }
        //update the status to DELIVERY_IN_PROGRESS(4)
        Orders orders = new Orders();
    ...
    public void reminder(Long id) {
        ...
        //give merchant a reminder by websocket
        Map map = new HashMap();
        map.put("type",2);  //1是来单提醒，2是客户催单。1 is a system's reminder for a new order，2 is customer's reminder for an existing order
        map.put("orderId", ordersDB.getId());
    ...
    private List<OrderVO> getOrderVOList(Page<Orders> page) {
        ...
        if(!CollectionUtils.isEmpty(ordersList)) {
            for(Orders order : ordersList){
                ...
                String orderStr = getOrderDishesStr(order);

                // 将订单菜品信息封装到orderVO中，并添加到orderVOList
                orderVO.setOrderDishes(orderStr);
    ...
    public void checkOutOfRange(String address){
        //api of baidu map，百度地图api介绍:https://lbsyun.baidu.com/faq/api?title=webapi/guide/webservice-geocoding-base
        Map map = new HashMap<>();
        map.put("address",shopAddress);
        map.put("output","json");
        map.put("ak",ak);

        //获取店铺的经纬度坐标get the shop's latitude and longitude
        ...
        //get coordinate of customer's address
        ...
        //解析数据analysis route data
        JSONObject result = jsonObject.getJSONObject("result");
        JSONArray jsonArray = (JSONArray) result.get("routes");
        Integer distance = (Integer) ((JSONObject) jsonArray.get(0)).get("distance");
        if(distance >5000){
            //配送距离超过5000米:delivery distance exceeds 5KM
            throw new OrderBusinessException("超出配送范围");   //if beyond delivery range,throw exception
        }
    }
}
